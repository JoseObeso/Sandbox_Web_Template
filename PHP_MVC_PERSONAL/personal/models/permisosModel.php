...
class permisosModel extends Model
{
	...
	public function getVerificarPermisos(usuario_web $u)
	{
	...
	public function getPlantillaModulo($dni,$modulo) {
		$sentencia = "select * from acceso_modulo where dni='".$dni."' and nombre='".$modulo."'";
		$resultado = $this->_db->prepare($sentencia);
		$resultado->execute();
	...
	public function CambiarPlantilla(acceso_modulo $am)
	{
		$sentencia = "update acceso_modulo set plantilla = ? where dni=? and nombre=?";
		$resultado = $this->_db->prepare($sentencia);
		$resultado->execute(array($am->getPlantilla(), $am->getDni(), $am->getNombre()));
		if($sentencia){
	...
	public function getVerificarMenu(acceso_submenu $a_submenu)
	{
		$sentencia = "select * from acceso_submenu where dni=?  and estado=1";
		$resultado = $this->_db->prepare($sentencia);
	...
	public function getListarMenuySubmenu($dni, $modulo)
	{
	...
	public function getListadoBotones(acceso_modulo $am,acceso_menu $a_menu,acceso_submenu $a_submenu)
	{
