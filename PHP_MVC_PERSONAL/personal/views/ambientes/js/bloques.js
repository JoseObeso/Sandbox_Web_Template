var url = 'http://' + document.domain + '/rrhh/personal/',
    ...
    leer_pisos, leer_idpisos, dato_listar_bloques, leer_idbloques, leer_idfuncional, leer_descripcion, leer_nro_cupos, leer_nombre_bloque_programacion, leer_idbloqueprogramacion, leer_nombre_bloque, ln_idcargoprofesion, leer_cargoprofesion, lc_inclusivo, ln_nro_profesional, leer_idasignacion, dato_listar_carrera, cantidad_registros, dato_eliminar_fun, dato_listar_funcion, leer_idasignacionbloque, lc_tipo_operacion_bloque, ln_caracter_de_ingreso_texto_nombre_bloques, lc_ver_si_contiene_af, lc_nombre_detalle;
...
function ver_bloques_programados() {
    ...
    $("#slt_lista_bloques_funcionales").select2({
        allowClear: true,
        placeholder: "Selecionar ",
        formatNoMatches: function (term) {
            var cont = $("<div />");
...
function leer_lista_bloques_funcionales() {
    ...
    $('#slt_lista_bloques_funcionales_detalle_muestra_bloque_cabecera')
        .html('')
        .attr("disabled", true);
...
function leer_nombre_detalle_bloque_ubicar_bloque_cabecera() {
    "use strict";
    lc_nombre_detalle = $('#slt_lista_bloques_funcionales_detalle  option:selected').attr('descripcion');
    var lc_ver_si_seleccion_nombre_detalle = (typeof lc_nombre_detalle === 'undefined') ? '' : '1';
    if (lc_ver_si_seleccion_nombre_detalle === '1') {
        var data_detalle = {
            'detalle': lc_nombre_detalle
        };
        $.ajax({
        data: data_detalle,
        dataType: 'json',
        url: url + 'ambientes/ver_cabecera_bloque_a_partir_de_detalle',
        type: 'post',
        beforeSend: function () {
            $("#espera_grabacion_personal").html("<img src='" + url_grafico + "/cargando.gif' width='128' height='15' alt=''/>");
        ...
        success: function (datos) {
            ...
            datos.forEach(function (filas) {
                if (filas.verificar === '1') {
                    $("#slt_lista_bloques_funcionales_detalle_muestra_bloque_cabecera")
                        .append('<option id="' + filas.id + '" nombre_bloque = "' + filas.nombre_bloque + '"  >' + filas.nombre_bloque + '</option>')
                        .attr("disabled", false);
...
// slt_lista_bloques_funcionales
    ...
    $("#slt_lista_bloques_funcionales_detalle")
        .click(function () {
            leer_nombre_detalle_bloque_ubicar_bloque_cabecera();
        })
        .keyup(function () {
            leer_nombre_detalle_bloque_ubicar_bloque_cabecera();
        })
    ...
    $("#slt_lista_bloques").change(function () {
        ...
        $("#txt_bloque_funcional").attr("disabled", false);
        ln_caracter_de_ingreso_texto_nombre_bloques = $("#txt_bloque_funcional").val().length;
        if (ln_caracter_de_ingreso_texto_nombre_bloques === 0) {
            $("#slt_lista_ambiente_funcional").attr("disabled", true);
            $("#txt_bloque_funcional").focus();

        } else {
            $("#slt_lista_ambiente_funcional").attr("disabled", false);

        }
        console.log(ln_caracter_de_ingreso_texto_nombre_bloques);
    ...
    $("#txt_bloque_funcional").keypress(function () {
        // console.log(lc_ver_si_contiene_af);
    ...
    $("#btn_modificar_bloque").click(function () {
        $("#slt_lista_ambiente_funcional_seleccion").attr("disabled", false);
        listar_pisos_fisico();
    });
