...
$(document).ready(function () {
    ...
    $("#txt_paterno")
        .keypress(function (e) {
            ...
            if (e.which === 13) {
                if ($("#txt_paterno").val().length === 0) {
                    $("#btn_grabar_datos_personales").attr("disabled", true);
                    $("#aviso_paterno").html('<center><font size="-1" color="#EF6000">Apellido Paterno vacio....</font></center>');
                } else {
                    $("#aviso_paterno").html('');
                    $("#txt_materno").focus();
                    if (lc_tipo_operacion_cas === '1') {
    ...
    $("#txt_nacimiento").change(function () {
        lc_fecha_nacimiento = convertir_fecha_caracter_a_date($("#txt_nacimiento").val());
        var ld_fecha_actual = new Date();
        var lc_edad = parseInt((ld_fecha_actual - lc_fecha_nacimiento) / 365 / 24 / 60 / 60 / 1000);
        $("#txt_edad").val(lc_edad);
        $("#slt_estado_civil")
            .attr("disabled", false)
            .focus();
    });
