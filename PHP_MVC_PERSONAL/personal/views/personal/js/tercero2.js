...
function leer_nombre_tercero_y_mostrar_resultado(lc_buscar_nombre) {
            ...
            if (nro_registros > 0) {
                encontrados.forEach(function (filas) {
                    if (filas.verificar === '1') {
                        ...
                        $('input:checkbox[name=chk_debaja]').attr('checked', false);
                    } else {
    ...
    $("#select_personal_existente").select2({
        placeholder: "Barra Espaciadora (Ver todos)",
        allowClear: true,
        language: "es",
        formatNoMatches: function (term) {
            ...
            var html = cont.html();
            return "<span>Personal No encontrado... </span> " + (term.length > 0 ? html : "");
        },
    });
    $("#select_personal_existente").select2("open");
    $("#select_personal_existente").addClass("alert");
...
function mostrar_personal_de_baja(lc_estado_personal) {
    ...
    $.ajax({
        ...
        success: function (encontrados) {
            $("#espera_proceso_datos_personales_terceros").html('');
            $("#listar_personal_de_baja").html('');
            nro_registros = encontrados.length;
            if (nro_registros > 0) {
                encontrados.forEach(function (filas) {
                    if (filas.verificar === '1') {
                        $("#listar_personal_de_baja").append('<option value="' + filas.idtercero + '"  id = "' + filas.idtercero +
                            '" idpais = "' + filas.idpais +
                            '" nombre_pais = "' + filas.nombre_pais +
                            '" tipo_documento = "' + filas.tipo_documento +
                            '" nro_documento = "' + filas.nro_documento +
                            ...
                            '" fecha_debaja = "' + filas.fecha_debaja +
                            '"  >' + filas.apellidosnombres + '</option>');
                        $("#listar_personal_de_baja").attr("disabled", false);
                        $("#total_registrados").html(nro_registros);
                        estadisticas_personal_tercero_habil_desactivado();
                        $("#chk_debaja").attr("disabled", false);
                    } else {
    ...
    $("#listar_personal_de_baja").select2({
        placeholder: "Barra Espaciadora (Ver todos)",
        allowClear: true,
        language: "es",
        formatNoMatches: function (term) {
    ...
    $("#listar_personal_de_baja").addClass("alert");
...
function leer_personal_tercero_mostrarlo_en_etiquetas() {
    ...
    if (lc_ver_si_tiene_id === '1') {
        ...
        leer_apellido_casada = $('#sslt_seleccionar_personal_terceros  option:selected').attr('apellido_casada');
...
function inicializar_botones_etiquetas_carrera_profesional_habilidad() {
    ...
    $("#btn_modificar_habilidad").attr("disabled", true);
...
function leer_habilidad_profesional_y_mostrarlo() {
    ...
    if (lc_ver_si_tiene_id === '1') {
        ...
        $("#btn_agregar_habilidad").attr("disabled", false);
        $("#btn_modificar_habilidad").attr("disabled", false);
...
*/
    ...
    $("#btn_modificar_habilidad").click(function () {
        lc_tipo_operacion_habilidad = '2';
        $('#fecha_inicio_habilitacion')
            .attr("disabled", false);
        $('#fecha_fin_habilitacion')
            .attr("disabled", false);
    ...
    $('#nro_de_habilitacion').keypress(function () {
        $("#btn_grabar_habilidad").attr("disabled", false);
    });
