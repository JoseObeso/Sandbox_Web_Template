...
function listar_programacion_mensual(leer_dni_personal_seleccion, leer_mes, leer_anio) {
    "use strict";
    data_programacion_listar_personal = {
        'dni': leer_dni_personal_seleccion,
        'mes': leer_mes,
        'anio': leer_anio
    };
    ...
    $.ajax({
        data: data_programacion_listar_personal,
        dataType: 'json',
        url: url + 'programacion/listar_programacion_mensual_por_personal',
        type: 'post',
        beforeSend: function () {
            $("#espera_crear_programacion").html("<img src='" + url_grafico + "/cargando.gif' width='128' height='15' alt=''/>");
        },
        success: function (encontrados) {
            ...
            if (nro_registros > 0) {
                encontrados.forEach(function (filas) {
                    if (filas.verifico === '1') {
                        $("#upss").text(filas.upss);
                        $("#departamento").text(filas.departamento);
                        $("#servicio").text(filas.servicio);
                        $("#slt_fechas_para_programacion")
                            .append('<option id="' + filas.id + '" fecha_turno = "' + filas.fecha_turno + '" dia = "' + filas.dia + '" dia_nombre = "' + filas.dia_nombre + '"  >' + filas.dia + ' - ' + filas.dia_nombre + '</option>');
                ...
                $("#slt_fechas_para_programacion").empty();
            }
...
function leer_fechas_y_mostrarlo() {
    "use strict";
    leer_idfechasprogramacion = $('#slt_fechas_para_programacion  option:selected').attr('id');
    lc_ver_si_seleccion_leer_idfechasprogramacion = (typeof leer_idfechasprogramacion === 'undefined') ? '' : '1';
    if (lc_ver_si_seleccion_leer_idfechasprogramacion === '1') {
        leer_idfechasprogramacion = $('#slt_fechas_para_programacion  option:selected').attr('id');
        leer_fecha_turno = $('#slt_fechas_para_programacion  option:selected').attr('fecha_turno');
        leer_dia = $('#slt_fechas_para_programacion  option:selected').attr('dia');
        leer_dia_nombre = $('#slt_fechas_para_programacion  option:selected').attr('dia_nombre');
        $("#slt_tipos_horarios_turnos").attr("disabled", false);
        $('#fecha_dia').text(leer_fecha_turno);
...
/*
function grabar_registro_programacion(leer_idfuncional, ln_horas_total, leer_hora_inicial, leer_hora_final, leer_horario_abreviado, leer_horario_mnt, leer_mes, leer_anio, leer_dni_personal_seleccion, lc_dia_seleccion) {
    "use strict";
    data_programacion_para_grabar = {
        'idfuncional': leer_idfuncional,
        'nhoras': ln_horas_total,
        'hora_inicial': leer_hora_inicial,
        'hora_final': leer_hora_final,
        'codigo_horario': leer_horario_abreviado,
        'codigo_turno': leer_horario_mnt,
        'dni': leer_dni_personal_seleccion,
        'mes': leer_mes,
        'anio': leer_anio,
        'dia': lc_dia_seleccion

    };

    $.ajax({
        data: data_programacion_para_grabar,
        dataType: 'json',
        url: url + 'programacion/grabar_datos_de_programacion_en_asistencia',
        type: 'post',
        beforeSend: function () {
            $("#espera_mientras_muestra_programacion").html("<img src='" + url_grafico + "/cargando.gif' width='128' height='15' alt=''/>");
        },
        success: function () {
            $("#espera_mientras_muestra_programacion").html("");

            $("#btn_registrar_horario_actividad").attr("disabled", false);
        },
        error: function (jqXHR, textStatus, errorThrown) {
            alert('Existe un ERROR ... Mira la consola (Ctrl + Shift + K) Pestaña : Consola para revisar!');
            $('#result').html('<p>codigo estado: ' + jqXHR.status + '</p><p>errorThrown: ' + errorThrown + '</p><p>jqXHR.responseText:</p><div>' + jqXHR.responseText + '</div>');
...
/*
function graba_actividad_con_horario(leer_id_upss_seleccion, leer_id_depar_seleccion, leer_id_upss_depar_servicio, leer_dni_personal_seleccion, leer_condicion_trabajo, leer_horario_abreviado, leer_horario_mnt, leer_idfuncional,lc_fecha_turno_para_datetime, lc_idactividad, lc_leer_dia, leer_mes, leer_anio, ln_horas_total, leer_hora_inicial, leer_hora_final){
    "use strict";
    data_grabar_actividad = {
        'idupss' : leer_id_upss_seleccion,
        'iddepar' : leer_id_depar_seleccion,
...
$(document).ready(function () {
    ...
    $("#slt_actividad").change(function () {
        lc_idactividad = $('#slt_actividad option:selected').attr('id');
        lc_nombre  = $('#slt_actividad option:selected').attr('nombre');
        lc_abreviatura  = $('#slt_actividad option:selected').attr('abreviatura');
        lc_titulo_actividad  = $('#slt_actividad option:selected').attr('titulo');
    ...
    $("#tipo_de_dia_0").click(function () {
        $("#slt_fechas_para_programacion").attr("disabled", false);
        lc_tipo_de_dia = '1';
    ...
    $("#slt_fechas_para_programacion").change(function () {
        $("#slt_nombre_dia").attr("disabled", true);
        lc_fecha_turno_para_datetime = $('#slt_fechas_para_programacion option:selected').attr('fecha_turno');
        lc_leer_dia = $('#slt_fechas_para_programacion option:selected').attr('dia');
