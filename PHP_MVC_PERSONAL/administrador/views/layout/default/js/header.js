var direccion_url = 'http://'+document.domain+'/rrhh/administrador/';
...
function mostrar_mensaje_en_modal(id_modal,tipo_alerta,mensaje_alerta,ocultar){
  ...
  if(ocultar==true){
    setTimeout(function(){
      $(""+id_modal).find(".mensajes").find('.mensajes-descripcion').slideUp('1000');
    },2000) ;
  } 
}
...
$(document).ready(function(){   
  ...
  $("#frm_cambiar_clave").validate({
    ...
    messages: {
      'clave_antigua': {
        required: 'Debe ingresar su clave antigua'
      },
      'nueva_clave': {
        required: 'Ingrese su nueva clave'
      },
      'confirmar_clave_nueva': {
        required: 'Clave vacia...',
  ...
  $(document).delegate("#frm_cambiar_clave","submit",function(){
    ...
    var posting = $.post(direccion_url + "modulos/cambiar_clave",datos);
