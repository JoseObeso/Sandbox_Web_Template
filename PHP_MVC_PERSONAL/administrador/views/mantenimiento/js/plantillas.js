var url = 'http://' + document.domain + '/rrhh/administrador/',
    ...
    boton_seleccionado = '',
    leer_id_modulo_actualizar, leer_nombre_modulo_actualizar,dato_actualizar;
...
function mensaje_emergente_aviso(aviso) {
    "use strict";
    var mensaje = aviso;
    bootbox.alert({
        size: "small",
        title: "<center><strong>:: I M P O R T A N T E:: </strong></center>",
        message: "<center><strong><font color='#D00033'>" + mensaje + "</font> <strong></center>",
        callback: function () {}
    });
}
    ...
    posting.done(function (data) {
        if (isJSON(data) == true) {
            ...
            if (imprimir[0]["estado_respuesta"] == 1) {
                ...
                mensaje_emergente_aviso('No existe grabacion');
            }
        } else {


            mensaje_emergente_aviso('Error del Sistema');
        }
    });
    posting.fail(function () {

        mensaje_emergente_aviso('No existe conexion con el servidor');
    })
}

function listar_menus() {
    var posting = $.post(url + "mantenimiento/getListadoPlantillaMenus", {
...
function listar_submenus() {
    var posting = $.post(url + "mantenimiento/getListadoPlantillaSubMenus", {
...
function listar_botones() {
    var posting = $.post(url + "mantenimiento/getListadoPlantillaBotones", {
...
$(document).ready(function () {
    ...
    $(document).delegate("#editar_modulo", "click", function () {
        if (modulo_seleccionado != '') {
            ...
            var posting = $.post(url + "mantenimiento/getDatosModulo", {
                id_pl_m: id_pl_m
            });
            posting.done(function (data) {
                if (isJSON(data) == true) {
    ...
    $(document).delegate("#editar_menu", "click", function () {
        if (menu_seleccionado != '') {
            ...
            var posting = $.post(url + "mantenimiento/getDatosMenu", {
                id_pl_menu: id_pl_menu
            });
            posting.done(function (data) {
                if (isJSON(data) == true) {
    ...
    $(document).delegate("#editar_submenu", "click", function () {
        if (submenu_seleccionado != '') {
            ...
            var posting = $.post(url + "mantenimiento/getDatosSubMenu", {
                id_pl_submenu: id_pl_submenu
            });
            posting.done(function (data) {
                if (isJSON(data) == true) {
        ...
        posting.done(function (data) {
            if (isJSON(data) !== true) {
                ...
                if (imprimir["estado_respuesta"] == 1) {
                    ...
                    mensaje_emergente_aviso('Todo Conforme');
                    listar_submenus();
                } else if (imprimir["estado_respuesta"] == 0) {
                    mensaje_emergente_aviso('Error de Grabación');
                }
            } else {
                mensaje_emergente_aviso('Error del sistema');
            }
