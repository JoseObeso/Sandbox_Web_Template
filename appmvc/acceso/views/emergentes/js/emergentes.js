 var modulo = "/appmvc",
     ...
     url_grafico_imagen = 'http://' + document.domain + '/images/emergentes/',
             ...
             if (data[0].respuesta == 1) {
                 if (data.length > 0) {
                     ...
                     for (var i = 0; i < data.length; i++) {
                         ...
                         datos_en_fila += '<td>' + (data[i].dimension) + '</td>';
                         datos_en_fila += '<td><button type="button"  id="' + data[i].idemergentes + '*' + data[i].archivo + '" class="btn btn-info  btn_ver_imagen" >Ver Img</button>' + '</td>';
                         datos_en_fila += '<td>' + (data[i].tipo_estado) + '</td>';
                         datos_en_fila += '<td><button type="button" name="activar" id="' + data[i].idemergentes + '" class="btn btn-success  btn_activar">Mostrar</button></td>';
                         datos_en_fila += '<td><button type="button" name="desactivar" id="' + data[i].idemergentes + '" class="btn btn-info  btn_desactivar">NO Mostrar</button></td>';
                         datos_en_fila += '<td><button type="button" name="eliminar" id="' + data[i].idemergentes + '" class="btn btn-danger  btn_eliminar">X</button></td>';
                         datos_en_fila += '</tr>';
                     }
                     $('#mostrar-datos-busqueda').append(datos_en_fila);
                     $('#msj-titular').html("<center><strong>Se ubicaron " + data.length + " -- Mensajes emergentes -- </center></strong>");
                 }
             } else {
 ...
 $(document).ready(function() {
     ...
     $("#imagen1").change(function() {
         ...
         if (subir_imagen1["type"] != "image/jpeg" && subir_imagen1["type"] != "image/png" && subir_imagen1["type"] != "image/jpg") {
             ...
             $.ajax({
                 dataType: 'json',
                 url: url + 'emergentes/subir_imagen1',
                 type: 'post',
                 data: data_form,
                 contentType: false,
                 cache: false,
     ...
     $("#btn_grabar").click(function() {
         ...
         if (lc_tipo_emergente === '') {
             $("#msj_grabar").html("<center><strong><font color='red'> -- NO SELECCIONO DIMENSIONES EN IMAGEN EMERGENTE --- </font><strong></center>").show();
             $("#btn_grabar").attr("disabled", true);
             $("#vertical")
                 .prop("checked", false)
                 .focus();
             ...
             var lc_txtDescripcion = $("#txtDescripcion").val(),
                 datos = {
                     ...
                     'tipo_dimension': lc_tipo_emergente
                 };
             $.ajax({
                 data: datos,
                 dataType: 'json',
                 url: url + 'emergentes/grabar',
                 type: 'post',
                 beforeSend: function() {
