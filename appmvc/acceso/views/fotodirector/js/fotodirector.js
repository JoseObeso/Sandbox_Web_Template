 var modulo = "/appmvc",
     ...
     url_ruta_archivos = 'http://' + document.domain + '/images/fotodirector/',
     ln_indicador = 0,
             ...
             if (data[0].respuesta == 1) {
                 if (data.length > 0) {
                     ...
                     for (var i = 0; i < data.length; i++) {
                         ...
                         datos_en_fila += '<td>' + (data[i].nombredirector) + '</td>';
                         datos_en_fila += '<td>' + (data[i].fotodirector) + '</td>';
                         datos_en_fila += '<td><a href = "' + (url_ruta_archivos + '/' + data[i].resolucion) + '" target = _black </a> ' + data[i].resolucion + '</td>';
                         datos_en_fila += '<td><button type="button"  id="' + data[i].id + '*' + data[i].fotodirector + '" class="btn btn-info  btn_ver_imagen" >Ver Img</button>' + '</td>';
 ...
 $(document).ready(function() {
     ...
     $("#imagen1").change(function() {
         ...
         if (subir_imagen1["type"] != "image/jpeg" && subir_imagen1["type"] != "image/png" && subir_imagen1["type"] != "image/jpg") {
             ...
             $.ajax({
                 dataType: 'json',
                 url: url + 'fotodirector/subir_imagen1',
                 type: 'post',
                 data: data_form,
                 contentType: false,
                 cache: false,
     ...
     $("#subir_archivo").change(function() {
         ...
         archivo_subir_bien = this.files[0],
             lc_nombre_archivo = archivo_subir_bien["name"],
         ...
         if (archivo_subir_bien["type"] != "application/pdf") {
             ...
             $.ajax({
                 dataType: 'json',
                 url: url + 'fotodirector/subir_pdf',
                 type: 'post',
                 data: data_form,
                 contentType: false,
                 cache: false,
     ...
     $("#btn_grabar").click(function() {
         var lc_txtDescripcion = $("#txtDescripcion").val(),
             datos = {
                 'nombre': lc_txtDescripcion,
                 'imagen1': lc_nombre_archivo_imagen1,
                 'resolucion': lc_nombre_archivo,
             };
         $.ajax({
             data: datos,
             dataType: 'json',
             url: url + 'fotodirector/grabar',
             type: 'post',
             beforeSend: function() {
