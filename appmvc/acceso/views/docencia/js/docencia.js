 var modulo = "/appmvc",
     ...
     url_ruta_archivos = 'http://' + document.domain + '/unidades/docencia/maq_docencia/images/galerias/',
     galerias_nombres = [],
 ...
 function listar_galeria() {
     ...
     $.ajax({
         dataType: 'json',
         url: url + 'docencia/listar_galerias',
         type: 'post',
         beforeSend: function() {
             ...
             if (data[0].respuesta == 1) {
                 if (data.length > 0) {
                     ...
                     for (var i = 0; i < data.length; i++) {
                         ...
                         datos_en_fila += '<td>' + (i + 1) + '</td>';
                         datos_en_fila += '<td>' + (data[i].fechagaleria) + '</td>';
                         datos_en_fila += '<td>' + (data[i].descripcion) + '</td>';
 ...
 $(document).ready(function() {
     ...
     $("#imagen-multiple").change(function() {
         ...
         for (i = 0; i < fileLength; i++) {
             ...
             if (!((imagefile == match[0]) || (imagefile == match[1]) || (imagefile == match[2]) || (imagefile == match[3]))) {
                 ...
                 $.ajax({
                     dataType: 'json',
                     url: url + 'docencia/multiples_imagenes',
                     type: 'post',
                     data: data_form,
                     contentType: false,
                     cache: false,
     ...
     $("#btn_grabar_galeria").click(function() {
         ...
         $.ajax({
             data: datos_grabar,
             dataType: 'json',
             url: url + 'docencia/grabar_galeria',
             type: 'post',
             beforeSend: function() {
                 $("#msj_imagen").html("<center><img src='" + url_grafico + "/cargando.gif' width='128' height='15' alt=''/></center>");
             },
     ...
     $("#si_eliminar_galeria").click(function() {
         ...
         $.ajax({
             data: data_eliminar,
             dataType: 'json',
             url: url + 'docencia/eliminar_galeria',
             type: 'post',
             beforeSend: function() {
