 var modulo = "/appmvc",
     ...
     url_ruta_archivos = 'http://' + document.domain + '/images/derecho/',
     ln_indicador = 0,
             ...
             if (data[0].respuesta == 1) {
                 if (data.length > 0) {
                     ...
                     for (var i = 0; i < data.length; i++) {
                         ...
                         datos_en_fila += '<td>' + (data[i].orden) + '</td>';
                         datos_en_fila += '<td><button type="button" name="cambiar" id="' + data[i].id + '" class="btn btn-primary  btn_cambiar_orden">Actualizar Orden</button></td>';
 ...
 $(document).ready(function() {
     ...
     $(document).on('click', '.btn_cambiar_orden', function() {
         var lc_leer_id = $(this).attr("id");
         $("#idorden").val(lc_leer_id);
         $('#mostrar_actualizar_orden').modal({ show: true, backdrop: 'static' });
     });
     ...
     $("#imagen1").change(function() {
         ...
         if (subir_imagen1["type"] != "image/jpeg" && subir_imagen1["type"] != "image/png" && subir_imagen1["type"] != "image/jpg" && subir_imagen1["type"] != "image/gif") {
             etiqueta_dom.val("");
     ...
     $("#grabar_orden").click(function() {
         ...
         $.ajax({
             ...
             success: function() {
                 $("#msj_imagen").html("");
                 $('#mostrar_actualizar_orden').modal("toggle");
                 listar();
             },
             error: function(jqXHR, textStatus, errorThrown) {
                 alert('Error, Revisar consola ');
