...
gn_numero_dia = (gd_fecha_total.getDay() == 0) ? 7 : gd_fecha_total.getDay();
...
function nombre_del_mes() {
    return nombre_mes[gn_mes - 1];
}

function numero_del_anio() {
    return gd_anio;
...
function primer_dia_mes_anio_real_time() {
    let primer_dia_mes_anio_real_time = '01/' + (('' + gn_mes).length < 2 ? '0' : '') + gn_mes + '/' + gd_anio;
    return primer_dia_mes_anio_real_time;
}
...
function mostrar_2_ultimos_digitos_anio() {

    return dia_mes_anio_real_time().substring(8);
