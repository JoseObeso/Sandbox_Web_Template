...
class centralModel extends Model
{
    ...
    public function Listar($descripcion, $anio, $mes)
    {
        ...
        $query = $query . " from central where 1 = 1 " . $filtro . $order;
    ...
    {
        $sql_update_registro = " insert into central(descripcion, archivo, enlace, estado, dfecharegistro, usuario) ";
    ...
    {
        if ($activar == '1') {
            $sql_update_registro = " update central set estado = '1' where id= " . $id;
        } else {
            $sql_update_registro = " update central set estado = '0' where id= " . $id;
        }
        $ejecucion = $this->_db->prepare($sql_update_registro);
        $ejecucion->execute();
        $verificar = array('verificar' => 1);
    ...
    {
        $sql_update_registro = " delete from central where id = " . $id;
        $ejecucion = $this->_db->prepare($sql_update_registro);
        $ejecucion->execute();
        $verificar = array('verificar' => 1);
        return json_encode($verificar);
