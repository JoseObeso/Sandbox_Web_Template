...
class estadisticaModel extends Model
{
    ...
    public function Listar()
    {
        ...
        $query = $query . " from calidad_documentos a left join calidad_categoria b on b.idcalidad_categoria = a.idcategoria  where a.tipo = 'T' order by a.vanio";
    ...
    {
        $query =  "select  idcalidad_documentos, fechadocumentos, descripcion, archivo, usuario, dfecharegistro, galerias from calidad_documentos  where tipo = 'E' order by vanio";
        $resultado = $this->_db->prepare($query);
        $resultado->execute();
        if ($resultado->rowCount()) {
            while ($imprimir = $resultado->fetch(PDO::FETCH_OBJ)) {
    ...
    {
        $query =  "select idcalidad_categoria, nombre, usuario, dfecharegistro from calidad_categoria where tipo = 'E';";
        $resultado = $this->_db->prepare($query);
        $resultado->execute();
        if ($resultado->rowCount()) {
            while ($imprimir = $resultado->fetch(PDO::FETCH_OBJ)) {
    ...
    public function Grabar($anio, $idcategoria, $descripcion, $archivo, $user)
    {
        ...
        $sql_update_registro = $sql_update_registro . " values ('".$anio."', '', ".$idcategoria.", upper('".$descripcion."'), '".$archivo."', '".$user."', now(), 'T') ";
    ...
    public function GrabarCategoria($categoria, $user)
    {
        ...
        $sql = $sql . "  values(upper('" . $categoria . "'), now(), '" . $user . "', 'E')";
        $ejecucion = $this->_db->prepare($sql);
        $ejecucion->execute();
        $verificar = array('verificar' => 1);
        return json_encode($verificar);
...
public function Listarbanner()
{
    ...
    $query = " select id, descripcion, archivo, estado,  case when estado = '1' then 'mostrar' else 'no' end as tipo_estado, dfecharegistro from banner where tipo = 'T'  " . $order;
    $resultado = $this->_db->prepare($query);
    $resultado->execute();
    if ($resultado->rowCount()) {
        while ($imprimir = $resultado->fetch(PDO::FETCH_OBJ)) {
...
public function Grabarbanner($descripcion, $imagen1, $dni)
{
    ...
    $sql_update_registro =  $sql_update_registro . "    values(upper('" . $descripcion . "') ,  '" . $imagen1 . "', '0', now(), 'T') ";
    $ejecucion = $this->_db->prepare($sql_update_registro);
    $ejecucion->execute();
    $verificar = array('verificar' => 1);
    return json_encode($verificar);
...
{
    $query = " select id, nombrejefe, cargojefe, estado, dfecharegistro,case when estado = '1' then 'MOSTRAR' else 'NO MOSTRAR' end as tipo_estado from banner where unidad = 'ST' ";
    $resultado = $this->_db->prepare($query);
    $resultado->execute();
    if ($resultado->rowCount()) {
        while ($imprimir = $resultado->fetch(PDO::FETCH_OBJ)) {
...
{
    $sql_update_registro =  " insert into banner (nombrejefe, cargojefe, unidad, estado, dfecharegistro) values (upper('" . $jefe . "'), upper('" . $cargo . "'), 'ST', '0', now())";
    $ejecucion = $this->_db->prepare($sql_update_registro);
    $ejecucion->execute();
    $verificar = array('verificar' => 1);
    return json_encode($verificar);
...
{
    $sql_update_registro = " update banner set estado = '1' where unidad = 'ST' and  id = " . $id;
    $sql_update_registro = $sql_update_registro . "; update banner set estado = '0' where unidad = 'ST' and  id <> " . $id;
    $ejecucion = $this->_db->prepare($sql_update_registro);
    $ejecucion->execute();
    $verificar = array('verificar' => 1);
    return json_encode($verificar);
...
{
    $query = " select id, nombrejefe, cargojefe, estado, dfecharegistro,case when estado = '1' then 'MOSTRAR' else 'NO MOSTRAR' end as tipo_estado from banner where unidad = 'RT' ";
    $resultado = $this->_db->prepare($query);
    $resultado->execute();
    if ($resultado->rowCount()) {
        while ($imprimir = $resultado->fetch(PDO::FETCH_OBJ)) {
...
{
    $sql_update_registro =  " insert into banner (nombrejefe, cargojefe, unidad, estado, dfecharegistro) values (upper('" . $jefe . "'), upper('" . $cargo . "'), 'RT', '0', now())";
    $ejecucion = $this->_db->prepare($sql_update_registro);
    $ejecucion->execute();
    $verificar = array('verificar' => 1);
    return json_encode($verificar);
...
{
    $sql_update_registro = " update banner set estado = '1' where unidad = 'RT' and  id = " . $id;
    $ejecucion = $this->_db->prepare($sql_update_registro);
    $ejecucion->execute();
    $verificar = array('verificar' => 1);
    return json_encode($verificar);
