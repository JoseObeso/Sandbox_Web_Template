...
class instrumentosModel extends Model
{
    ...
    public function Listar()
    {
        ...
        $query = $query . " from calidad_documentos a left join calidad_categoria b on b.idcalidad_categoria = a.idcategoria  where a.tipo = 'Y' order by a.vanio";
    ...
    {
        $query =  "select idcalidad_categoria, nombre, usuario, dfecharegistro from calidad_categoria where tipo = 'Y';";
        $resultado = $this->_db->prepare($query);
        $resultado->execute();
        if ($resultado->rowCount()) {
            while ($imprimir = $resultado->fetch(PDO::FETCH_OBJ)) {
    ...
    public function Grabar($anio, $idcategoria, $descripcion, $archivo, $user)
    {
        ...
        $sql_update_registro = $sql_update_registro . " values ('".$anio."', '', ".$idcategoria.", upper('".$descripcion."'), '".$archivo."', '".$user."', now(), 'Y') ";
    ...
    public function GrabarCategoria($categoria, $user)
    {
        ...
        $sql = $sql . "  values(upper('" . $categoria . "'), now(), '" . $user . "', 'Y')";
